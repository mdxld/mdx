...
interface MockExecutionContext {
  ...
  on: (event: string, callback: (data: any) => Promise<void>) => any;
  db: {
    build: any;
    watch: any;
    get: any;
...
vi.mock('../../../packages/mdxe/cli/src/utils/execution-context', () => {
  const createMockList = (): ListFunction => {
    const mockItems = ['Item 1', 'Item 2', 'Item 3']
    
    const listFn = async function(): Promise<string[]> {
      return mockItems
    } as ListFunction
    
    listFn[Symbol.asyncIterator] = async function*() {
  ...
  return {
    createExecutionContext: vi.fn((): MockExecutionContext => {
      ...
      const mockResearch = vi.fn(() => Promise.resolve({
      ...
      const mockList = vi.fn(() => createMockList())
...
describe('Startups Example Integration', () => {
  ...
  it('should execute the full startups workflow', async () => {
    ...
    await on('idea.captured', async (idea) => {
      ...
      const listFn = list`10 possible market segments for ${idea}`
