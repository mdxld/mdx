 ...
 */
export const EventStatusProvider: React.FC<EventStatusProviderProps> = ({
  children,
  onEventUpdate
}) => {
  ...
  const findEventById = (
    eventId: string,
    eventList: EventStatusItem[] = events
  ): { event: EventStatusItem; path: number[] } | undefined => {
  ...
  const updateEventAtPath = (
    path: number[],
    updates: Partial<EventStatusItem>,
    eventList: EventStatusItem[]
  ): EventStatusItem[] => {
...
export const EventSystemConnector: React.FC<EventSystemConnectorProps> = ({
  children,
  eventRegistry
}) => {
