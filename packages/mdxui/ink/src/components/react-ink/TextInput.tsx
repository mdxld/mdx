...
function findPrevWordJump(prompt: string, cursorOffset: number) {
  ...
  const stringToCursorOffset = prompt.slice(0, cursorOffset).replace(/[\s,.;!?]+$/, '')
    ...
    (input: string, key: any) => {
      ...
      if (key.return) {
      ...
      } else if (key.ctrl && input === 'w') {
        {
          ...
          nextValue = originalValue.slice(0, cursorOffset - cut) + originalValue.slice(cursorOffset)
        ...
        const stringToCursorOffset = originalValue.slice(0, cursorOffset).replace(/[\s,.;!?]+$/, '')
        ...
        nextValue = stringToCursorOffset.slice(0, lastMatch) + originalValue.slice(cursorOffset, originalValue.length)
