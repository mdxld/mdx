      ...
      <div className='container px-4 md:px-6 mx-auto'>
        <div className='grid gap-6 lg:grid-cols-2 lg:gap-12 items-center'>
          <div className='flex flex-col justify-center space-y-4'>
            {badge && <div className='inline-block rounded-full bg-gray-100 dark:bg-gray-800 px-3 py-1 text-sm'>{badge}</div>}
            <h1 className='text-3xl font-bold tracking-tighter sm:text-5xl xl:text-6xl/none'>{headline}</h1>
            <p className='max-w-[600px] text-gray-500 dark:text-gray-400 md:text-xl'>{description}</p>
...
export const Problem = forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement> & ProblemSection>(
  ({ className, headline, description, points, imageUrl, imageAlt, badge, ...props }, ref) => (
    <section ref={ref} className={cn('w-full py-12 md:py-24 lg:py-32 bg-gray-50 dark:bg-gray-900', className)} {...props}>
      <div className='container px-4 md:px-6 mx-auto'>
        <div className='grid gap-6 lg:grid-cols-2 lg:gap-12 items-center'>
          {imageUrl && (
          ...
          <div className={cn('space-y-4', imageUrl ? 'order-1 lg:order-2' : '')}>
            ...
            {description && <p className='max-w-[600px] text-gray-500 dark:text-gray-400 md:text-xl'>{description}</p>}
...
export const FeatureCard = forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement> & FeatureItem>(
  ({ className, title, description, iconUrl, iconAlt, ...props }, ref) => (
    <div ref={ref} className={cn('rounded-lg border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-gray-950', className)} {...props}>
    ...
    <section ref={ref} className={cn('w-full py-12 md:py-24 lg:py-32', className)} {...props}>
      <div className='container px-4 md:px-6 mx-auto'>
        <div className='flex flex-col items-center justify-center space-y-4 text-center'>
          {badge && <div className='inline-block rounded-full bg-gray-100 dark:bg-gray-800 px-3 py-1 text-sm'>{badge}</div>}
          {headline && <h2 className='text-3xl font-bold tracking-tighter sm:text-4xl md:text-5xl'>{headline}</h2>}
          {description && <p className='max-w-[700px] text-gray-500 dark:text-gray-400 md:text-xl'>{description}</p>}
        </div>
  ...
  ({ className, name, price, description, features, ctaText, ctaLink, featured, ...props }, ref) => (
    ...
    >
      <div className='p-6 space-y-2'>
        {featured && <div className='inline-block rounded-full bg-black px-3 py-1 text-sm text-white dark:bg-white dark:text-black mb-2'>Popular</div>}
  ...
  ({ className, headline, description, plans, note, badge, ...props }, ref) => (
    <section ref={ref} className={cn('w-full py-12 md:py-24 lg:py-32', className)} {...props}>
      <div className='container px-4 md:px-6 mx-auto'>
        ...
        {note && <div className='mt-8 text-center text-sm text-gray-500 dark:text-gray-400'>{note}</div>}
      </div>
    </section>
  ),
)
      ...
      <div className='flex flex-col h-full'>
        ...
        <div className='flex items-center gap-4 mt-4 pt-4 border-t border-gray-200 dark:border-gray-800'>
          ...
          <div>
            ...
            {authorTitle && <p className='text-sm text-gray-500 dark:text-gray-400'>{authorTitle}</p>}
          </div>
        </div>
      </div>
    </div>
...
export const FAQAccordion = forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement> & FAQItem & { isOpen: boolean; toggleOpen: () => void }>(
  ({ className, question, answer, isOpen, toggleOpen, ...props }, ref) => (
    <div ref={ref} className={cn('border-b border-gray-200 dark:border-gray-800', className)} {...props}>
      <button onClick={toggleOpen} className='flex w-full items-center justify-between py-4 text-left font-medium transition-all'>
        <span>{question}</span>
        <svg
          xmlns='http://www.w3.org/2000/svg'
          width='24'
        ...
        <div className='container px-4 md:px-6 mx-auto'>
          ...
          <div className='mx-auto max-w-3xl mt-8'>
            {faqs.map((faq: FAQItem, i: number) => (
              <FAQAccordion key={i} {...faq} isOpen={openIndex === i} toggleOpen={() => setOpenIndex(openIndex === i ? null : i)} />
            ))}
          </div>
          {primaryActionText && primaryActionLink && (
...
export const CallToAction = forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement> & CallToActionSection>(
  ({ className, headline, description, primaryActionText, primaryActionLink, secondaryActionText, secondaryActionLink, badge, ...props }, ref) => (
    <section ref={ref} className={cn('w-full py-12 md:py-24 lg:py-32 bg-black text-white dark:bg-white dark:text-black', className)} {...props}>
      <div className='container px-4 md:px-6 mx-auto'>
        <div className='flex flex-col items-center justify-center space-y-4 text-center'>
          {badge && <div className='inline-block rounded-full bg-white/20 px-3 py-1 text-sm text-white dark:bg-black/20 dark:text-black'>{badge}</div>}
          <h2 className='text-3xl font-bold tracking-tighter sm:text-4xl md:text-5xl'>{headline}</h2>
          {description && <p className='max-w-[700px] text-white/80 dark:text-black/80 md:text-xl'>{description}</p>}
...
export const TeamMemberCard = forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement> & TeamMember>(
  ({ className, name, role, photoUrl, photoAlt, bio, ...props }, ref) => (
    <div ref={ref} className={cn('overflow-hidden rounded-lg border border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-950', className)} {...props}>
      {photoUrl && (
