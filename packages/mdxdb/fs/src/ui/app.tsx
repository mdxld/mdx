...
async function findSchemaFile(dir: string): Promise<string | null> {
  try {
    ...
    const schemaFiles = files.filter((file) => file === 'README.md' || file === 'SCHEMA.md' || file === 'SCHEMA.mdx')
    ...
    messages: [
      {
        role: 'system',
        content:
          'You are a schema generator. Generate a JSON schema for a collection based on the description. The schema should include a name property and a schema object with fields array.',
      },
...
async function generateDocumentContent(collectionSchema: any, description: string): Promise<any> {
  ...
  const result = await generateText({
    ...
    messages: [
      {
        role: 'system',
        content: 'You are a document generator. Generate a JSON document based on the schema description. Include a "body" field with the main content.',
      },
            ...
            queueManager.addTask(`Generating database with ${numCollections} collections`, async () => {
              for (let i = 0; i < numCollections; i++) {
                ...
                queueManager.addTask(`Generating schema for ${collectionName}`, async () => {
                  const schema = await generateCollectionSchema(`${description} - Collection ${i + 1} of ${numCollections}`)
                  ...
                  const readmeContent = matter.stringify(`# ${schema.name} Collection\n\nThis collection was generated by mdxdb.`, { schema: schema.schema })
            ...
            queueManager.addTask(`Generating ${count} documents for collection: ${collectionName}`, async () => {
              ...
              for (let i = 0; i < count; i++) {
                queueManager.addTask(`Generating document ${i + 1} of ${count}`, async () => {
                  const document = await generateDocumentContent(schema, `${description} - Document ${i + 1} for collection ${collectionName}`)
