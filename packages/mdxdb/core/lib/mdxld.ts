 ...
 */
export async function buildWithMdxld(options: { sourceDir?: string; outputDir?: string; configFile?: string; watch?: boolean }): Promise<any> {
  const { sourceDir = '.', outputDir = '.mdx', configFile, watch = false } = options

  console.log('Building MDX files using mdxld...')

  const args = ['mdxld', 'build', '--source', sourceDir, '--output', outputDir]
 ...
 */
export function watchWithMdxld(options: { sourceDir?: string; outputDir?: string; configFile?: string }): ChildProcess {
  const { sourceDir = '.', outputDir = '.mdx', configFile } = options

  const args = ['mdxld', 'build', '--source', sourceDir, '--output', outputDir, '--watch']
