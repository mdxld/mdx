  ...
  describe('research command', () => {
    ...
    it('should execute research command in non-interactive mode', async () => {
      ...
      expect(fs.writeFileSync).toHaveBeenCalledWith(
        'test-output.mdx',
        expect.stringContaining('# Research Results'),
        'utf-8'
      )
    ...
    it('should handle parameter passing from command line', async () => {
      ...
      await program.parseAsync([
        'node', 'test', 'research', 'How do I use AI?', 
        '-o', 'custom-output.mdx',
        '-f', 'both'
      ])
    ...
    it('should throw an error when AI_GATEWAY_TOKEN is not set', async () => {
      ...
      await expect(
        program.parseAsync(['node', 'test', 'research', 'How do I use AI?'])
      ).rejects.toThrow('Process.exit called with code: 1')
      
      expect(mockConsoleError).toHaveBeenCalledWith(
        'AI_GATEWAY_TOKEN environment variable is not set.'
      )
    })
  })
})
