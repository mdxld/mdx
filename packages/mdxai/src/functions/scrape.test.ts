...
describe('scrapeMultiple', () => {
  ...
  beforeEach(() => {
    ...
    // Setup mock implementation for scrape
    mockScrape.mockImplementation(async (url) => {
      if (url.includes('error')) {
        return {
          url,
          error: 'Failed to scrape: Scraping failed',
    ...
    // Replace the mocked scrape function
    vi.mocked(scrape).mockImplementation(mockScrape);
  });
