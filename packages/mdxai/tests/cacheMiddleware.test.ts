...
import { createCacheMiddleware, CacheConfig } from '../src/cacheMiddleware.js'
...
describe('Cache Middleware', () => {
  ...
  it('should use file cache when configured', async () => {
    ...
    const mockReadFile = vi.fn()
      .mockRejectedValueOnce(new Error('File not found')) // First call fails (cache miss)
      .mockResolvedValueOnce(JSON.stringify({ ...mockResult, timestamp: Date.now() })); // Second call succeeds (cache hit)
    ...
    expect(result2).toHaveProperty('text', 'test response')
    expect(mockDoGenerate2).not.toHaveBeenCalled()
  })
