 ...
 */
export async function generateChatResponseWithMCP(messages: ChatMessage[], mcpClients: MCPClient[]): Promise<AsyncGenerator<StreamPart>> {
  try {
    ...
    if (Object.keys(aggregatedTools).length === 0) {
      return mockStreamGenerator('No MCP tools are available. Please add an MCP source first.', true, false)
    }
    ...
    return mockStreamGenerator(`Sorry, I encountered an error while using MCP tools: ${error instanceof Error ? error.message : String(error)}`, true, false)
  }
}
...
export async function generateChatResponseWithSearch(messages: ChatMessage[]): Promise<AsyncGenerator<StreamPart>> {
  try {
    ...
    if (lastUserMessage.content.toLowerCase().includes('mdx')) {
      ...
      response =
        'I searched the web for information related to your query. MDX is a powerful format that combines Markdown with JSX, allowing developers to write JSX directly in their Markdown documents. This enables the inclusion of React components within content, making it ideal for interactive documentation and content-rich applications.'
    }
