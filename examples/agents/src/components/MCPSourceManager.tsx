...
export const MCPSourceManager: React.FC<MCPSourceManagerProps> = ({ 
  onAddSource,
  disabled = false
}) => {
  ...
  return (
    <Box flexDirection="column" marginY={1}>
      <Box>
        <Text bold color="green">Add MCP Source: </Text>
        <Text>{value}</Text>
      ...
      <Box marginTop={1}>
        <Text dimColor>
          Enter a URL for SSE transport (https://...) or a command with args for stdio transport (node server.js)
        </Text>
      </Box>
    </Box>
  );
